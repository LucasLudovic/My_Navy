...
static
int reset_ping_values(int increment, int *state_changed, int *number_of_ping)
{
...
int retrieve_ping(int increment)
{
    ...
    reset_ping_values(increment, &state_changed, &number_of_ping);
    if (state_changed == 1) {
        state_changed = 0;
        return END_RETRIEVE;
    }
...
static
int play_single_turn(player_t *player)
{
...
int loop(player_t *player)
{
    ...
    while (game_on) {
        ...
        if (play_single_turn(player) == FAILURE)
            return FAILURE;
        usleep(1000);
    }
    return SUCCESS;
